/**
  ******************************************************************************
  * @file    stm32h750_discovery_sd.c
  * @author  MCD Application Team
  * @brief   This file includes the uSD card driver mounted on STM32H750_DK
  *          boards.
  @verbatim
  How To use this driver:
  -----------------------
   - This driver is used to drive the micro SD external cards mounted on STM32H750_DK
     board.
...
LOG_MODULE_REGISTER(bsp_sd, LOG_LEVEL_DBG);

/** @addtogroup BSP
 * @{
 */

/** @addtogroup STM32H750_DK
 * @{
 */

/** @defgroup STM32H750_DK_SD SD
 * @{
 */

/** @defgroup STM32H750_DK_SD_Private_TypesDefinitions Private TypesDefinitions
 * @{
 */
...
/** @defgroup STM32H750_DK_SD_Private_Variables Private Variables
 * @{
 */
...
/** @defgroup STM32H750_DK_SD_Exported_Variables Exported Variables
 * @{
 */
...
/** @defgroup STM32H750_DK_SD_Private_Functions_Prototypes Private Functions Prototypes
 * @{
 */
...
/** @addgroup STM32H750_DK_SD_Exported_Functions Exported Functions
 * @{
 */
...
/** @defgroup STM32H750_DK_SD_Private_Functions Private Functions
 * @{
 */
#if (USE_HAL_SD_REGISTER_CALLBACKS == 1)
/**
...
int32_t bap_sd_init(void)
{
	int32_t ret = 0;
	ret = BSP_SD_Init(BSP_SD_INSTANCE);
	if (ret != BSP_ERROR_NONE) {
		LOG_WRN("BSP_MMC_Init ret = %d", ret);
		return ret;
	} 
	 
	ret = BSP_SD_GetCardState(BSP_SD_INSTANCE);
    if (ret != BSP_ERROR_NONE) {
	    LOG_WRN("BSP_SD_GetCardState ret = %d", ret);
        return ret;
    }
 
	BSP_SD_CardInfo CardInfo;
	ret = BSP_SD_GetCardInfo(BSP_SD_INSTANCE, &CardInfo);
	if (ret != BSP_ERROR_NONE) {
		LOG_WRN("BSP_MMC_GetCardInfo err = %d", ret);
	} else {
		LOG_DBG("CardInfo.BlockNbr = %d, %d", CardInfo.BlockNbr, CardInfo.BlockSize);
		uint64_t memory_size_mb = (uint64_t)CardInfo.BlockNbr * CardInfo.BlockSize;
